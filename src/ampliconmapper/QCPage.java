...
public class QCPage implements Page{
	...
	public QCPage(Stage primaryStage, File FastQCLocation, File BBToolsLocation, File PicardLocation, File TabletLocation, String javaCall) {
	...
	public void run() {
		...
		readFileField.setTooltip(new Tooltip("A directory containing only files in .fastq.gz, .fasta, .fa, .fna, or .ffn format"));
...
//Aka fastq, sam, and bam files. 
	...
	private class FastQCCall extends Task<Void>{
		...
		protected Void call() {
			...
			AccessoryMethods.logMessage(outputField, "Threads: " + threads);
			...
			AccessoryMethods.logMessage(outputField, "\nRelocating files to output directory");
	...
	private class FastQCTask extends Task<Void>{
		...
		@Override
		protected Void call() {
			AccessoryMethods.logMessage(outputField, FastQCLocation.getAbsolutePath());
			BlockingQueue<String> messageQueue = new LinkedBlockingQueue<>();
			consumer = new MessageConsumer(messageQueue, this.outputField);
			String[] fullProcessCall;
			if(System.getProperties().getProperty("os.name").contains("Windows")) {
				fullProcessCall = new String[] {javaCall, "-Xmx500m", "-classpath", ".;./sam-1.103.jar;./jbzip2-0.9.jar", "uk.ac.babraham.FastQC.FastQCApplication",
